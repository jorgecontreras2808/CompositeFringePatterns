...
lowFreqImagesPath = strcat(filepath, strcat(strcat('\AquiredLowFreq',string(steps)),'\'));
highFreqImagesPath = strcat(filepath, strcat(strcat('\AquiredHighFreq',string(steps)),'\'));

correctionParams = load([[userpath, '\APIs\Calibration\'],'correctionParams.mat']);
[imagesHighFreq, nImages] = ImageHelper.LoadChannelImages(highFreqImagesPath,'.png', ColorChannel.Red);
[imagesLowFreq, ~] = ImageHelper.LoadChannelImages(lowFreqImagesPath,'.png', ColorChannel.Red);
...
imagesHighFreq = ImageHelper.RemoveImagesDoubleDistortion(imagesHighFreq, correctionParams.correctionParams, nImages, 1);
imagesLowFreq = ImageHelper.RemoveImagesDoubleDistortion(imagesLowFreq, correctionParams.correctionParams, nImages, 1);
...
for i = 1: capturedPhases
    ...
    images = imagesLowFreq(:,:,imageIndex:imageIndex+steps-1);
